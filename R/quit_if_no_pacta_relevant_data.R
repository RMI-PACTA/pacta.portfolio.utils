...
quit_if_no_pacta_relevant_data <- function(portfolio) {
  has_pacta_relevant_holdings <-
    (
      any(
        portfolio$asset_type == "Equity" & portfolio$has_asset_level_data,
        na.rm = TRUE
      ) ||
        any(
          portfolio$asset_type == "Bonds" & portfolio$has_ald_in_fin_sector,
          na.rm = TRUE
        )
    )

  if (!has_pacta_relevant_holdings) {
